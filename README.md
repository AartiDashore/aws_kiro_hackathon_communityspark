...
- edit all business fields (name, description, story, category, address, phone, website) via the ✏️ Edit Business button
- upload a photo (JPEG, PNG, WebP — max 5 MB) stored in `static/uploads/`
- remove the uploaded photo — cards fall back to a category emoji placeholder automatically
...
- edit a deal after posting — title, description, discount, price, and time adjustment
- extend or shorten remaining time using the "Adjust time" field (positive = extend, negative = shorten)
- set an urgency threshold: when X hours remain, a sticky banner fires at the top of the page — "⚡ Hurry up! Only Xh Ym remaining before [deal] expires!" — dismissible by the visitor
- delete a deal manually at any time with the 🗑️ button on the deal card
